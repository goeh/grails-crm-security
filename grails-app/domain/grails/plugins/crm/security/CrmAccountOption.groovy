 ...
 * Account option.
 */
class CrmAccountOption<T> {
    String key
    String v
    static belongsTo = [account:CrmAccount]
    static constraints = {
        key(maxSize: 80, unique: 'account')
        v(maxSize: 2000, blank: true)
    }

    static mapping = {
        table 'crm_account_option'
        sort "key"
    ...
    CrmAccountOption(String key, Object value) {
        this.key = key
        this.setValue(value)
    }

    CrmAccountOption(CrmAccount account, String key, Object value) {
